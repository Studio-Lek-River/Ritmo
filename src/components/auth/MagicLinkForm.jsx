...
import { sendMagicLink } from '../../sync/auth';
...
export default function MagicLinkForm({ theme, onSubmitted, onSignInWithPassword }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <h3 className={`font-semibold ${theme.textSecondary}`}>{t('auth.magicLinkTitle')}</h3>
        <p className={`text-sm ${theme.textMuted}`}>{t('auth.magicLinkDescription')}</p>
      </div>
